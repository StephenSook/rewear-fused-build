...
- **Beat 2, Engine C, the working floor (25s)** [Passport]: "This is live. A compliance classifier trained on 631 real CPSC recall records routes every garment clear, lab-test, or divert against actual US and EU regulation, then emits a scannable passport. Scan it." [hand them the QR; the live single-product passport opens on their phone] "And it is cryptographically verifiable, watch." [click Verify, the green Authentic shield, then Tamper, it flips red] "The browser checks the Ed25519 signature live, no server; tamper a field and it breaks. That is the piece Carter's deploys in year one."
...
- **Beat 4, Engine B, the redesigned hydrolase (25s)** [Mol* live]: "Our swing was a de-novo carbamate hydrolase with no prior art; that GPU run we deferred. What we designed in-silico is the LigandMPNN-redesign fallback. What is rotating is the CALB serine-hydrolase scaffold, PDB 1TCA, the published template our lead design is a redesign of: it shows the exact Ser-His-Asp pocket, repositioned for carbamate hydrolysis at near-neutral pH so it spares the cotton. In-silico PLACER preorganization targets on the Lauko thresholds, FoldSeek TM about 0.8 to the template. Honestly a redesign, not a de-novo fold. Download the reference scaffold to inspect the active-site geometry."
...
- **Q4, "Can AI really design a new block copolymer?"** "Two honest answers. On the enzyme: our swing was a de-novo fold with no prior art, but that GPU run we deferred, so what we designed is the LigandMPNN-redesign fallback, three LigandMPNN redesigns with repositioned Ser-His-Asp triads and in-silico PLACER targets, a redesign at TM about 0.8 to template, not a de-novo fold. On the fiber we never claim generative invention: it's constraint-guided screening, we enumerate, score, and filter known chemistries. Both honest and defensible. Here's the funnel." [point at the funnel]
