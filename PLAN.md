...
| 4.1 | Wire frontend to real artifacts + live endpoint | frontend, data | Stephen | 🟡 | 3.3 | Engine C wired to real signed bundle AND a live `/classify` client (NEXT_PUBLIC_ENGINE_C_API_URL) with demo-safe 2.5s fallback + live/signed-bundle badge; all 3 states verified, silent-failure review folded in. Live data flows once Vinh deploys 3.4; Engine A/B still in-silico |
| 4.2 | Deploy (June 14 fresh repo) | infra | all | 🟡 | 1.3 | de-risked: vercel.json + private preview RENDERS on Vercel (Mol*, real data, 0 errors). Public deploy + Render endpoint + secrets at June-14 |
...
- **2026-06-13 (Stephen):** Frontend integration — the passport (demo floor) now classifies against the **live Engine C endpoint** when `NEXT_PUBLIC_ENGINE_C_API_URL` is set (`engine-c-client.ts` + `useEngineC` hook), with a visible "Engine C · live / signed bundle" provenance badge + model version, and a demo-safe 2.5s AbortController fallback to the signed bundle (never hangs/breaks). Verified all 3 states in-browser against the real local FastAPI: live (badge live, server hit), fallback (badge signed bundle, no hang), default (no fetch, 0 errors). Ran a `silent-failure-hunter` review and folded in its fixes (per-artifact source so a live classify shows even if the passport lags; runtime shape guard against malformed live data under a "live" badge; terminal `.catch` + dev logging). Commit c8bd43b, CI green. Goes live automatically once Vinh deploys 3.4.
- **2026-06-13 (Stephen):** Demo-safety wave (commits ddacfcd, b030294, d7907d3; CI green). (1) **Deterministic demo mode** (CLAUDE.md §7.14, cannot-be-dropped): Shift+D / Esc / `?demo=1` — a cancellable loop auto-walks Story→Passport→Fiber→Enzyme→Loop, driving each scroll-narrative scene with a controlled Lenis scroll, on-brand HUD, loops so it never dead-ends; it's also the OBS-capture source. (2) **Error boundaries** so a crash can't white-screen the demo: route `error.tsx` + root `global-error.tsx` (inline-styled) + `SceneBoundary` around the 4 heavy 3D mounts (a WebGL failure degrades to a label / silent dark, not a blank view) + a WebGL probe on the fiber scene. (3) On-brand 404 + programmatic OG image + share metadata for the deployed link. Verified: demo mode auto-walks (0 errors), golden-path 5/5, OG 200/png, 404 proper status. Ran a 2-agent adversarial review (demo-mode lifecycle + boundaries) and folded in all fixes (scrollHeight stability poll, HUD dwell timing, SceneBoundary dev logging, fiber WebGL probe).
- **2026-06-13 (Stephen):** ⚠️ **CONTRACT change + artifact-conformance gate** (commits ee615c7 contract, c4f3427 validator; CI green). Reconciled two drifts the new validator surfaced: `garments.json` lives at the artifact root (not `garments/`), and the `swim` archetype is now in the contract enum (already in types.ts). **PING Pravin + Vinh: conform to these going forward (cosmetic).** Built `scripts/validate_artifacts.py` — a CI gate that enforces every contract shape + the locked scientific constraints (aliphatic isocyanate, Ser-His-Asp + pH 7.0-8.5, 25-35 wt% hard segment, designWindow [25,35], pair cross-refs, pdbPath resolves, manifest SHA parity + reverse-coverage). This is the automated **June-15 freeze gate** (run `--strict`): it catches drift the moment Vinh's / Pravin's real artifacts land, so "just connect everything" can't silently break the frontend. `build.py` now signs the FULL bundle (garments + regulations) so the manifest covers every artifact. Verified conformant on the real bundle + proven to catch violations; folded in a `code-reviewer` audit (added screening/tradeoff validators, manifest reverse-coverage, pdbPath/SHAP/trl checks).
- **2026-06-13 (Stephen):** Interactive-instrument chrome across all 6 views. Upgraded the single `BeveledBox` so every instrument panel (the corner-tick boxes in Story / Fiber / Enzyme / Loop / Passport / Judges, ~30 of them) is now cursor-reactive: corner ticks extend into locking brackets on hover, an accent spotlight tracks the pointer (painted behind the text via `isolation:isolate` + negative-z so the fine mono readout stays crisp), a reticle line sweeps the panel, the frame blooms, and the box leans in 3D toward the cursor with a magnetic drift; clickable boxes (garment cards, the substrate-handoff CTA) get a tactile press. Extracted the GSAP tilt into a shared `usePointerTilt` hook (Tilt3D now uses it too). Canvas/chart boxes (Mol*, R3F fiber, the trade-off curve) keep the chrome but `tilt={false}` so a parent transform never wobbles their own pointer math. Two-accent system carried through (bio-green enzyme, amber fiber); reduced-motion safe. Pattern confirmed against 21st.dev spotlight/highlighter snippets. Verified: enzyme view hover in-browser (corner brackets lock, spotlight behind text, 0 console errors), tsc + lint clean, prod webpack build green, golden-path 5/5 (CI mode).
- **2026-06-13 (Stephen):** Full-repo audit (7-agent fan-out) + fixes (commits 80eecc7…1dac513; CI green, 5 jobs). Verdict: core is sound — data real, PR-AUC non-circular, no secrets/leaks/slop, contract chain self-consistent. Fixed every real finding: **honesty overclaims** (the /judges card claimed 4 datasets, only CPSC wired → cut to CPSC + roadmap; "85+ thresholds" → "real US/EU regulation"; Engine B PLACER metrics labelled illustrative/pending); **real bugs** (GTIN was salted-`hash()` non-deterministic + failed its GS1 check digit → deterministic valid GTIN-14; did:key now spec-compliant `z6Mk…`; `build_manifest.py` was clobbering modelVersions/issuer → now preserves them); **strictness** (enabled `noUncheckedIndexedAccess` + `verbatimModuleSyntax` per §3, fixed ~25 index accesses incl. the latent `garments[1]`); **perf** (ScrollProgress off the scroll event, not a free-running rAF); **honest naming** (`shapTopFeatures`→`compositionDrivers`); dead-code removed. **June-14 public-repo safety:** `init_event_repo.sh` now excludes CLAUDE.md, PLAN.md, the lane handoffs, demo script, devpost text, event runbook (internal strategy / IP-FTO / judge names / secrets) — research-reports was already excluded. Deployed Vercel preview re-verified interactive (passport, QR, Mol*, 0 errors).
- **2026-06-13 (Stephen):** Galaxy-tier interaction + 3D-glow wave, grounded in a deep-research pass (`/deep-research`, 107 agents, 25 claims adversarially verified) and a 5-agent R3F-scene audit (commits d28e7fa, 9118860, + this one; CI green). (1) **Interactive instrument chrome** on the single `BeveledBox` so all ~30 corner-tick boxes are cursor-reactive (corner ticks INK on via staggered draw, spotlight tracks the pointer behind the text via isolate + negative-z, reticle sweep, bloom, 3D tilt + magnetic, press on clickables); a single perpetual **border-beam** on the one hero (the enzyme Mol* frame). (2) **3D scene glow upgrades** the audit surfaced, each verified in-browser + WebGL2-only + graceful: fiber-architecture now seats in depth (accent-split Sparkles + fog + Vignette) with richer amber hard segments kept SUBORDINATE so the cleavable bond stays the one glowing hero (I overrode the audit's "make the fiber bloom" because it inverted the Constraint-3 science hierarchy); **Mol\* selective emissive bloom** makes only the Ser-His-Asp triad energize bio-green (grounded in the installed molstar 5.9 param shapes, isolated in its own try/catch so a mismatch degrades to the plain cartoon, never a dead viewer); the two additive shader clouds (story braid, loop storm) get brighter HDR-ish per-particle cores in-shader (NO composer: both are fixed transparent overlays where a composer risked blacking the page / washing the headline). Dropped per the audit + demo-safety: Mol* path-traced illumination, DepthOfField, the story-seed HDR punch (headline legibility), all WebGPU/TSL. Verified: all 5 views 0 console errors, headline legible, tsc + lint clean, prod build green, golden-path 5/5.
- **2026-06-13 (Stephen):** Pulled the three "deferred to polish week" items forward (owner directive: best possible project, no risk-deferral). All verified in-browser + golden-path 5/5. (1) **Composer-bloom on the two transparent overlay scenes** (story braid, loop storm): empirically confirmed `@react-three/postprocessing` EffectComposer PRESERVES the canvas alpha (no blacked page), so I removed the interim in-shader core boost (composer now owns the glow) and tuned gentle + high-threshold (intensity 0.4-0.45, luminanceThreshold 1.0) so only the brightest fuse pixels halo and the centered headlines stay legible (re-verified "Today's onesie" + "Safe to wear, able to be remade" both readable). Gated behind `!reduced`. (2) **Mol\* SSAO** (contact-shadow depth on the enzyme cartoon), explicit full molstar-5.9 occlusion params, in its OWN try/catch separate from the bloom so a param mismatch can't take the bloom down. (3) **Glowing corner brackets** (`drop-shadow` accent on the locked ink-on corners): the genuine DrawSVG-look upgrade. Noted honestly: for STRAIGHT L-brackets a literal SVG stroke-draw is pixel-identical to the shipped CSS draw-on, so the real improvement is the glow, not DOM churn. Context-exhaustion note: hammering ~10 Playwright navigations in one browser triggered a Mol* `MAX_VERTEX_TEXTURE_IMAGE_UNITS` degraded-context error; golden-path (fresh context per view) is clean, so it's a test-harness artifact, not an app regression.
- **2026-06-13 (Stephen):** Interactive CTA buttons + 6-agent deep repo review. (A) New reusable `InstrumentButton` (magnetic + cursor spotlight + charge-stripe sweep + accent bloom + sliding arrow; `download` renders a real `<a download>`, `stream` adds a downloading data cascade): migrated the 3 loop cross-nav CTAs (THE FIBER/ENZYME/PASSPORT) and the .pdb download onto it. (B) **Deep review** (6 parallel agents: pr-review code-quality, front-to-back wiring, silent-failure-hunter, Gemini whole-repo, Codex adversarial, honesty) → **verdict: ZERO blockers, ZERO highs; product sound + wired front-to-back on real data; honesty moat holds; no fabricated data in the judged path.** Fixed all 6 MEDIUM (verified against source first): molstar setStatus-after-dispose race (split conditions); `usePointerTilt` cleanup now `gsap.killTweensOf` (no orphaned tween on the ticker) + leave-scale only when it owns scale; molstar `.bcif/.pdb` format inferred from the url (no silent blank when the binary CIF lands); removed the dead `audio-engine` cleavage-sample branch + its missing-asset JSDoc; `mock-data.pdbPath` forward-looking comment; softened the SourceBadge tooltip ("signed offline bundle", since no runtime browser SHA check happens). Plus: **full em-dash sweep** (18 files, ZERO remaining: prose to period/colon, page+OG titles + numbered eyebrows to the project's `·` separator) per the AI-tone rule before the public repo; honesty relabel of the download to "Reference scaffold .pdb"; hoisted the duplicated `EASE` constant to `lib/motion.ts`; hoisted `REFERENCE_PDB` to `lib/data.ts`. Noted (not changed): engine-c rules.py dead features (Vinh's lane), `NEXT_PUBLIC_ENGINE_C_API_URL` must be set at Vercel deploy for the live badge (ops). Verified: tsc + lint clean, prod build green, golden-path 5/5, /judges copy re-verified in-browser (0 errors). Commits 315dc01 (buttons) + this one; CI green.
